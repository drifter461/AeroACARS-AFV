...
- **Phase-Strings-Vollständigkeit** (Contract Sektion 1.3): Die 18-Werte-Liste im Contract ist verbindlich. Diese Spec führt exakt 3 neue Werte ein (REJECTED_TAKE_OFF, GO_AROUND, DEBOARDING) — keine weiteren ohne Contract-Update.
- **Owner: StateMachine** (Contract Sektion 3): FlightStats `stats.phase` ist alleiniger Schreiber. Discord-RPC, Webapp, GlitchTip dürfen NUR lesen.
- **Persistenz** (Contract Sektion 4): `rto_event` und `go_around_events` im `pireps.payload_json` (= JSON-Blob, KEIN eigenes Schema-Feld). PIREP-Notes-Append zusätzlich für Pilot-lesbare Story.
- **Backward-Compat** (Contract Sektion 5): Alte Webapp-Versionen rendern neue Phase-Strings via Fallback `t(\`phase.${value}\`, { defaultValue: value })`. Alte phpVMS-Konsumenten ignorieren `rto_event`/`go_around_events` stillschweigend.
- **Feature-Flags** (Contract Sektion 7): 3 separate Flags `phases.rejected_take_off_enabled`, `phases.go_around_enabled`, `phases.deboarding_enabled` (alle Default `true`)
- **Bau-Reihenfolge** (Contract Sektion 6): Diese Spec ist Step 2, NACH Delta-Sampling (Step 1, weil Phase-Force-Publish dort eingeführt wird) und VOR Per-Bounce (Step 3, das LANDED als Anker nutzt)
- **Versionierungs-Strategie** (Contract Sektion 11): Neue PIREP-Felder `rto_event`/`go_around_events` als `Option<...>` mit `#[serde(skip_serializing_if = "Option::is_none")]`
- **E2E-Tests** (Contract Sektion 8): G3 (RTO), G4 (Doppel-GO_AROUND), G5 (Pause/Slew), G7 (alter PIREP), G10 (Opt-Out) testen erweiterte Phasen
...
### Vergleich Stratos v2.14.0 (NUR-Referenz, NICHT AeroACARS-Wire-Contract)

> ⚠️ **WICHTIG — diese Sektion ist Stratos-Referenz, nicht AeroACARS-Wirklichkeit.**
> Die Strings unten (`UNKNOWN`, `PUSH_BACK`, `TAXI`, `TAKE_OFF`, `LANDED` etc.) sind die internen Stratos-Phase-Namen aus deren Codebase. Sie unterscheiden sich teilweise von unseren kanonischen Phase-Strings (`PREFLIGHT`, `PUSHBACK`, `TAXI_OUT`, `TAKEOFF_ROLL`, `LANDING` etc.).
>
> **Verbindlich für AeroACARS sind ausschließlich die 18 Phase-Strings aus [`v0.9.0-telemetry-contract.md`](v0.9.0-telemetry-contract.md) Sektion 1.3.** Diese Spec fügt davon GENAU 3 neue Werte hinzu: `REJECTED_TAKE_OFF`, `GO_AROUND`, `DEBOARDING`. Die unten zitierten Stratos-Werte werden NICHT übernommen — sie dienen ausschließlich als Architektur-Inspiration für die Trigger-Werte (45 kt, 8°/70 %, Engines-off etc.).
>
> Konkrete Mapping-Tabelle Stratos→AeroACARS am Ende dieser Sektion.

Stratos (kommerzieller ACARS-Konkurrent, Electron/TypeScript) hat 17 Phasen, sehr ähnlich zu unseren — aber explizit mit RTO/GA/Deboarding. Auszug aus deren `FlightPhase.ts`:

```
// STRATOS-INTERN (NICHT AeroACARS!):
...
**Stratos→AeroACARS-Wire-Contract-Mapping** (= falls jemand fragt „warum nicht 1:1"):

| Stratos-String | AeroACARS-String (kanonisch laut Contract Sektion 1.3) | Anmerkung |
|---|---|---|
| `UNKNOWN` | (nicht übernommen) | AeroACARS startet immer in `PREFLIGHT`, kein UNKNOWN-Zwischenzustand |
| `BOARDING` | `BOARDING` | identisch |
| `PUSH_BACK` | `PUSHBACK` | AeroACARS schreibt ohne Underscore — bestehend, nicht geändert |
| `TAXI` | `TAXI_OUT` | AeroACARS unterscheidet TAXI_OUT vs TAXI_IN, Stratos nicht |
| `TAKE_OFF` | `TAKEOFF_ROLL` + `TAKEOFF` | AeroACARS hat zwei Sub-Phasen (Rollen + Lift-off), Stratos eine |
| `REJECTED_TAKE_OFF` | **`REJECTED_TAKE_OFF`** (NEU v0.9.0) | exakt übernommen |
| `CLIMB`/`CRUISE`/`DESCENT`/`APPROACH`/`FINAL` | identisch | — |
| `LANDED` | `LANDING` | AeroACARS-Begriff bestehend |
| `GO_AROUND` | **`GO_AROUND`** (NEU v0.9.0) | exakt übernommen |
| `TAXI_IN`/`ARRIVED` | identisch | — |
| (Stratos hat nicht) | `BLOCKS_ON` | AeroACARS-spezifisch, bestehend |
| `DEBOARDING` | **`DEBOARDING`** (NEU v0.9.0) | exakt übernommen |

**Verbindlich ist NUR die rechte Spalte.** Die Stratos-Strings werden im AeroACARS-Code nirgendwo geschrieben, gelesen oder verarbeitet — sie sind reine Vergleichsreferenz.
